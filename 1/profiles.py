...
r = [np.sqrt(x**2 + y**2 + z**2) for x, y, z in s.gas['pos'][s.gas['num_recycled'] > -1]]
nrc = s.gas['num_recycled'][s.gas['num_recycled'] > -1]
z = s.gas['metals'][s.gas['num_recycled'] > -1]/s.gas['mass'][s.gas['num_recycled'] > -1]

nrc_avg, edges_nrc, count_nrc = stats.binned_statistic(r, nrc, statistic='mean', bins=np.linspace(0, 3000, 300))
z_avg, edges_z, count_z = stats.binned_statistic(r, z, statistic='mean', bins=np.linspace(0, 3000, 300))
