...
for i, args in enumerate(utils.combinations):
    ...
    if i == 0:
        MgII_profiles[0] = pg.analysis.radially_binned(s.gas, 'temp', av='MgII', r_edges=bins)
        MgII_profiles[1] = pg.analysis.radially_binned(s.gas[m_rec], 'temp', av='MgII', r_edges=bins)
        MgII_profiles[2] = pg.analysis.radially_binned(s.gas[m_nrec], 'temp', av='MgII', r_edges=bins)
        OVI_profiles[0] = pg.analysis.radially_binned(s.gas, 'temp', av='OVI', r_edges=bins)
        OVI_profiles[1] = pg.analysis.radially_binned(s.gas[m_rec], 'temp', av='OVI', r_edges=bins)
        OVI_profiles[2] = pg.analysis.radially_binned(s.gas[m_nrec], 'temp', av='OVI', r_edges=bins)
    else:
        MgII_profiles[0] = np.column_stack((MgII_profiles[0], pg.analysis.radially_binned(s.gas, 'temp', av='MgII', r_edges=bins)))
        MgII_profiles[1] = np.column_stack((MgII_profiles[1], pg.analysis.radially_binned(s.gas[m_rec], 'temp', av='MgII', r_edges=bins)))
        MgII_profiles[2] = np.column_stack((MgII_profiles[2], pg.analysis.radially_binned(s.gas[m_nrec], 'temp', av='MgII', r_edges=bins)))
        OVI_profiles[0] = np.column_stack((OVI_profiles[0], pg.analysis.radially_binned(s.gas, 'temp', av='OVI', r_edges=bins)))
        OVI_profiles[1] = np.column_stack((OVI_profiles[1], pg.analysis.radially_binned(s.gas[m_rec], 'temp', av='OVI', r_edges=bins)))
        OVI_profiles[2] = np.column_stack((OVI_profiles[2], pg.analysis.radially_binned(s.gas[m_nrec], 'temp', av='OVI', r_edges=bins)))

y_label = pg.analysis.radially_binned(s.gas, 'temp', av='MgII', r_edges=bins).units.latex()
