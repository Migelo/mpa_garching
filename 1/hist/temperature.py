...
ax[0].scatter(s.gas['T_at_infall'][s.gas['num_recycled'] > -1], s.gas['T_at_ejection'][s.gas['num_recycled'] > -1])

ax[1].set_ylabel("count")
ax[1].set_xlabel("$T$ [K]")
ax[1].hist(T_infall, bins=np.linspace(0, 1.2e4, 100), alpha=.5, label='infall')
ax[1].hist(T_ejection, bins=np.linspace(0, 1.2e4, 100), alpha=.5, label='ejection')
plt.legend(loc='best')
